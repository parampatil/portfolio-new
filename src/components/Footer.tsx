...
const Footer = () => {
  ...
  return (
    <motion.footer
      ...
      ref={ref}
      className="border-t border-gray-900 bg-black p-8 text-white"
    >
      <motion.div
        className="mx-auto grid-col-1 grid gap-4 text-left sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 max-w-5xl"
        variants={containerVariants}
      >
        {navItems.map((section) => (
          <motion.div
            ...
            className="mb-6"
          >
            <h3 className="mb-4 text-lg font-bold text-aurora-orange">
              <Link
                to={section.path}
                className="transition-colors hover:text-aurora-red"
              >
                {section.name}
              </Link>
            </h3>
